...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        unsigned workGroupSize = 16;
        unsigned global_work_size = (width * height + workGroupSize - 1) / workGroupSize * workGroupSize;
        gpu::gpu_mem_32f image;
        image.resizeN(width * height);
        timer t;
        gpu::WorkSize size(workGroupSize, workGroupSize, (width + workGroupSize - 1) / workGroupSize * workGroupSize, (height + workGroupSize - 1) / workGroupSize * workGroupSize);
        for (int i = 0; i < benchmarkingIters; i++) {
            kernel.exec(size,
                        image,
                        width, height,
        ...
        image.readN(gpu_results.ptr(), width * height);
    }
